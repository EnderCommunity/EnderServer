...
var xmlhttp2 = new XMLHttpRequest(), xmlhttp3 = new XMLHttpRequest(), processResponse = function(isUsername, repsonse){
  if(isUsername){
    if(repsonse == "true"){
      //This account doesn't exist, you can move on!
      document.getElementById("username").style.display = "none";
      document.getElementById("password").style.display = "block";
    }else{
      //This account already exists!
      usernameInputError(true, "This username is already taken!");
    }
  }else{
    if(repsonse != "false"){
...
}, usernameInput = document.getElementsByName("usernameInput")[0], usernameInputErrorText = document.getElementsByClassName("usernameInput")[0], alt = document.getElementById("alt"), usernameInputError = function(isError, text = "[Error_Message]"){
  if(isError){
...
}, passwordInput = document.getElementsByName("passwordInput")[0], passwordInputErrorText = document.getElementsByClassName("passwordInput")[0], passwordInputError = function(isError, text = "[Error_Message]"){
  if(isError){
...
document.getElementById("_next1").addEventListener("click", function(){
  doWhenLoading();
  if(!usernameInput.value.match(/^[0-9a-zA-Z_]+$/) && usernameInput.value != ""){
    ...
    usernameInputError(true, "Please enter your new username!");
    doWhenDoneLoading();
  }else{
    try{
